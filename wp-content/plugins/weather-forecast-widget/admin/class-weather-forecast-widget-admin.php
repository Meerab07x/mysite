...
class Weather_Forecast_Widget_Admin {
	...
	public function enqueue_scripts() {
		...
		wp_register_script( 'sweetalert2', 'https://cdn.jsdelivr.net/npm/sweetalert2@11', array(), null, true );
	...
	public function wfw_add_contribute_section() {
		return;
	}
	public function wfw_render_donate_button() {
		$title = esc_js( __( 'Thanks for your support!', 'weather-forecast-widget' ) );
		$text = esc_js( __( 'If you like the plugin, you can donate here easily and securely.', 'weather-forecast-widget' ) );
		$button_text = esc_js( __( 'PayPal donation page', 'weather-forecast-widget' ) );
		?>
		<button type="button" id="donateBtn" class="btn btn-primary" style="margin-top:10px;">
			❤️ Support via PayPal
		</button>
		<script>
		document.addEventListener('DOMContentLoaded', function () {
			document.getElementById('donateBtn').addEventListener('click', function(event) {
				event.preventDefault();	// Verhindert z.B. das Neuladen, falls Button in Formular ist
				Swal.fire({
				  ...
				  preConfirm: () => {
					window.open('https://paypal.me/bergtourentipptirol', '_blank');
				  }
				});
			});
		});
	...
	public function wfw_render_rating_button() {
		$title = esc_js( __( 'Thanks for your support!', 'weather-forecast-widget' ) );
		$text = esc_js( __( 'If you like the plugin, you can simply leave a review here.', 'weather-forecast-widget' ) );
		$button_text = esc_js( __( 'Submit a review', 'weather-forecast-widget' ) );
		?>
		<button type="button" id="ratingBtn" class="btn btn-primary" style="margin-top:10px;">
			🌟 <?php echo __('Enjoying the plugin? Leave a 5-star review!', 'weather-forecast-widget'); ?>
		</button>
		<script>
		document.addEventListener('DOMContentLoaded', function () {
			document.getElementById('ratingBtn').addEventListener('click', function(event) {
				event.preventDefault();	// Verhindert z.B. das Neuladen, falls Button in Formular ist
				Swal.fire({
				  title: "<?php echo $title; ?>",
				  text: "<?php echo $text; ?>",
				  ...
				  preConfirm: () => {
					window.open('https://wordpress.org/support/plugin/weather-forecast-widget/reviews/#new-post', '_blank');
				  }
				});
			});
		});
	...
	public function wfw_add_settings_field_input_text( $args ) {
		...
		if ( $field_id == 'openweathermap-apikey' && esc_attr( $option ) ) {
			...
			if ( $http_status === 200 ) {
				$msg = sprintf(__('<p style="color: green">API KEY <strong>%s</strong> is VALID!</p>', 'weather-forecast-widget' ), esc_attr( $option ) );
				echo $msg;
				
				?>
				<div style="margin-top: 1em;">
					<button type="button" onclick="var el = this.nextElementSibling; el.style.display = el.style.display === 'none' ? 'block' : 'none';">
						📂 Rohdaten anzeigen / ausblenden
					</button>
					<pre style="display: none; background: #f6f6f6; padding: 1em; border: 1px solid #ccc; max-height: 300px; overflow: auto;"><?php echo esc_html( json_encode( $test_apikey, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE ) ); ?></pre>
				</div>
				<?php
			} else {
				$msg = sprintf(__('<p style="color: red">API KEY <strong>%1$s</strong> is NOT VALID: %2$s | %3$s</p>', 'weather-forecast-widget' ), esc_attr( $option ), $test_apikey->cod, $test_apikey->message );
				echo $msg;
			}
		} elseif ( $field_id == 'nominatim-citytest' && esc_attr( $option ) ) {
			$test_nominatim = $this->wfw_test_nominatim_api(esc_attr( $option ));
			if ( $test_nominatim['success'] ) {
				$msg = sprintf(__('<p style="color: green">Coordinates for city <strong>%s</strong> found !</p>', 'weather-forecast-widget' ), esc_attr( $option ) );
				echo $msg;
				$msg = sprintf(__('<p style="color: green">Latitude: <strong>%s</strong></p>', 'weather-forecast-widget' ), esc_html( $test_nominatim['data']['lat'] ) );
				echo $msg;
				$msg = sprintf(__('<p style="color: green">Longitude: <strong>%s</strong></p>', 'weather-forecast-widget' ), esc_html( $test_nominatim['data']['lon'] ) );
				echo $msg;
				
				if ( !empty( $test_nominatim['data']['raw'] ) ) {
					$raw_data = print_r( $test_nominatim['data']['raw'], true );
					?>
					<div style="margin-top: 1em;">
						<button type="button" onclick="var el = this.nextElementSibling; el.style.display = el.style.display === 'none' ? 'block' : 'none';">
							📂 Rohdaten anzeigen / ausblenden
						</button>
						<pre style="display: none; background: #f6f6f6; padding: 1em; border: 1px solid #ccc; max-height: 300px; overflow: auto;"><?php echo esc_html( $raw_data ); ?></pre>
					</div>
					<?php
				}
			} else {
				echo '<p style="color: red"><strong>' .esc_html( $test_nominatim['message'] ) .'</strong></p><br />';
			}
		}
	}
	...
	public function wfw_register_settings() {
		...
		add_settings_section(
			$this->plugin_name . '-contribute-section',
			__( 'Contribute', 'weather-forecast-widget' ),
			array( $this, 'wfw_add_contribute_section' ),
			$this->plugin_name . '-settings'
		);
		
		add_settings_field(
			'donate_button',
			__( 'Donate', 'weather-forecast-widget' ),
			array( $this, 'wfw_render_donate_button' ),
			$this->plugin_name . '-settings',
			$this->plugin_name . '-contribute-section'
		);
		...
		add_settings_field(
			'rating_button',
			__( 'Rating', 'weather-forecast-widget' ),
			array( $this, 'wfw_render_rating_button' ),
			$this->plugin_name . '-settings',
			$this->plugin_name . '-contribute-section'
		);
		...
		add_settings_field(
			'nominatim-citytest',
			__( 'Test Nominatim query', 'weather-forecast-widget' ),
			array( $this, 'wfw_add_settings_field_input_text' ),
			$this->plugin_name . '-settings',
			$this->plugin_name . '-settings-section',
			array(
				'label_for' => 'nominatim-citytest',
				'default' => 'Berlin',
				'placeholder' => __( 'Enter a city with which you want to test the Nominatim query.', 'weather-forecast-widget' ),
			)
		);
	...
	public function wfw_test_apikey() {
		...
		$apiUrl = 'https://api.openweathermap.org/data/3.0/onecall?lat=52.518399&lon=13.399939&exclude=minutely&units=metric&lang=de&appid=' . $apikey;
		//echo $apiUrl;	//debugging
	...
	public function wfw_test_nominatim_api($city) {
		if (!$city) {
			return [
				'success' => false,
				'message' => __( 'A city must be entered so that the query can be tested', 'weather-forecast-widget' ),
				'data'    => null,
			];
		}

		// Nominatim URL mit Query-Parameter
		...
		// User-Agent ist wichtig, sonst wird die Anfrage abgelehnt
		$options = [
			"http" => [
				"header" => "User-Agent: weather-forecast-widget/1.0\r\n"
			]
		];
		...
		if ($response === FALSE) {
			return [
				'success' => false,
				'message' => __( 'Error in API request', 'weather-forecast-widget' ),
				'data'    => null,
			];
		}
		...
		if (empty($data)) {
			return [
				'success' => false,
				'message' => sprintf(__('No results found for %s.', 'weather-forecast-widget' ), $city ),
				'data'    => null,
			];
		}
		...
		return [
			'success' => true,
			'message' => __( 'Coordinates successfully found.', 'weather-forecast-widget' ),
			'data'    => [
				'lat' => $lat,
				'lon' => $lon,
				'raw' => $data[0], // optional: gesamte Antwort
			]
		];
	}
}
