...
class Weather_Forecast_Widget_Actions {
	...
	public function wfw_get_weather_widget( $post_id, $template, 
		...
		if ( $city ) {
			...
			$data = "";
			*/
			$NominatimCacheValue = 'wfw_nominatim_' . $city;
			$NominatimApiResponse = $this->wfw_call_nominatim_api($NominatimCacheValue, $cache_time, $city);
			if ( $NominatimApiResponse['success'] ) {
				$lat = $NominatimApiResponse['data']['lat'];
				$lon = $NominatimApiResponse['data']['lon'];
			} else {
				return $this->error_msg( esc_html( $NominatimApiResponse['message'] ) );
			}
		};
		if ( !$apiUrl && $lat && $lon ) {
			...
			$apiUrl =	'https://api.openweathermap.org/data/3.0/onecall?lat=' . $lat . '&lon=' . $lon . '&exclude=minutely&units=' .$weather_unit .'&lang=' . $lang_code . '&appid=' . $apiKey;
	...
	private function wfw_call_nominatim_api($cache_value, $cache_time, $city) {
		if (!$city) {
			return [
				'success' => false,
		...
		if ($cache_data && $cache_data['success'] ) {
